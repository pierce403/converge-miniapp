...
After XMTP is ready, the browser rechecks the Farcaster wallet's current XMTP network assignment and sends its bounded `Allowed` group-topic/HMAC snapshot through two Quick Auth routes. Mini forces the exact callback URL, generic/minimal-payload preferences, and its own random FID-scoped `inboxHandle`; keeps the vapid.party app secret server-side; and returns vapid.party's exact `signatureText` for `Client.signWithInstallationKey`. The browser signs that text with the current installation, and vapid.party verifies XMTP's RFC 8032 Ed25519ph proof with the fixed `PUBLIC SIGNATURE CONTEXT` before atomically keeping one active installation for `(appId, inboxHandle)`. Plain Ed25519 is incompatible. The Mini D1 never stores the XMTP inbox ID, installation ID, topics, HMAC keys, proof, ticket, or management receipt.

For delivery, vapid.party's singleton Go process consumes the global legacy XMTP `production` `SubscribeAll` stream. Its current in-memory index contains only registered topic/HMAC routes. It ignores unmatched or noncanonical topics, `shouldPush: false`, and self-authored envelopes identified by sender HMAC; it neither decrypts nor forwards the MLS envelope. A match becomes a deduplicated at-least-once Queue job whose only Mini-facing content is a stable delivery ID and opaque handle.
