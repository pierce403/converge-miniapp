...
- Alert promotion remains fail-closed. The notification-encryption secret and three vapid.party app secrets are present; the current-Hub credential remains unconfigured. Configuring all five notification secrets is necessary but not sufficient for promotion: while committed `FARCASTER_NOTIFICATIONS_ENABLED` is `"false"`, `GET /api/notifications/status` remains `{"available":false}`, the manifest omits `webhookUrl`, and the UI must not prompt. The signed lifecycle webhook, installation-proof enrollment proxy, signed opaque callback, fixed native delivery, invalid-token cleanup, and logical-route revocation still require canonical-host proof before a reviewed config change enables rollout.
...
Keep `FARCASTER_NOTIFICATIONS_ENABLED` at the committed exact string `"false"` in production and preview while configuring credentials and running integration checks. This public-availability gate is deliberately separate from bridge configuration: controlled callers can exercise the unadvertised signed webhook, Quick Auth enrollment routes, and signed callback while the status endpoint, manifest, and browser prompt remain off. Values such as `"TRUE"`, `"1"`, or whitespace-padded `"true"` do not enable rollout. Preview remains unavailable even with the flag and credentials because the structural bridge is production-only.

Before promotion, prove signed add, rotation, disable/removal, token-pending `425`, installation replacement, signed callback replay, invalid-token deletion, upstream throttling, and app-side route revocation. Review D1 only for ciphertext, opaque handles, and delivery IDs, and inspect sampled logs for token, URL, FID, signature, topic, HMAC, inbox, or installation leakage. Confirm production still returns `{"available":false}` and omits `webhookUrl` with every secret configured. Only after those checks pass, make a reviewed config-only change setting the production `FARCASTER_NOTIFICATIONS_ENABLED` value to the exact string `"true"` while leaving preview `"false"`, push through Cloudflare Workers Builds, and verify both `{"available":true}` and the exact `https://miniapp.converge.cv/api/farcaster/webhook` manifest value. Revert the production variable to `"false"` to withdraw public availability; this rollout gate does not erase existing encrypted tokens or replace route-level revocation.
