...
- Alert promotion is structurally live as of 2026-07-27. The current-network, notification-encryption, and three vapid.party secret names are configured; the exact DNS binding resolves publicly; production `FARCASTER_NOTIFICATIONS_ENABLED` is `"true"` while preview remains `"false"`; `GET /api/notifications/status` returns `{"available":true}`; and the manifest publishes the exact canonical webhook. Farcaster's public debugger validates the manifest and ownership proof. Bounded manual redirects within `neynar.com` repaired current-network verification: Worker `a639f300-5b00-44fd-b675-b9897e4fcfb2` returns `400` for the unauthorized-key canary, a real enable event creates exactly one encrypted native subscription and one active opaque route, and no webhook failure is logged. The open-app flow now reaches vapid.party ticket issuance. Sanitized production diagnostics report exact upstream `409`, which the live version-5 contract defines as an unverified callback app domain, while public DNS and the app key still match. On this one result, Mini reads the private app-domain state, sets only its server-controlled canonical domain when needed, verifies the TXT record, and retries the ticket once. It never exposes provider bodies, registrations, credentials, tickets, identities, topics, FIDs, signatures, notification URLs, or tokens. Require successful ticket/proof enrollment before treating closed-app delivery or cleanup as operational.
...
The current alert-diagnostic checkpoint shipped through Cloudflare Workers Builds in commit `cd64cac` and immutable Worker version `d33d418b-4d98-409d-953f-7db97c38d146`. Production returns `{"available":true}`, the manifest contains `https://miniapp.converge.cv/api/farcaster/webhook`, and the live asset contains the fixed `notification_token_pending` diagnostic. Registration failures expose only an allowlisted stage, code, and HTTP status; arbitrary first-party response bodies and caught SDK details are not rendered. Complete current-Hub credential repair, signed add, rotation, disable/removal, installation replacement, one closed-app delivery, callback replay, invalid-token deletion, upstream throttling, and app-side route revocation acceptance. Review D1 only for ciphertext, opaque handles, and delivery IDs, and inspect sampled logs for token, URL, FID, signature, topic, HMAC, inbox, installation, sender, or message leakage. Reverting production to `"false"` withdraws public availability but does not erase existing encrypted tokens or replace route-level revocation.
