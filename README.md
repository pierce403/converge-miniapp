...
The production payer Gateway and final Farcaster account-association values remain unconfigured. Until account association is configured, `/.well-known/farcaster.json` serves a metadata-only bootstrap manifest with `noindex: true` and no ownership claim; partial or invalid association configuration fails closed. Until the Gateway is configured, production XMTP messaging also fails closed.
