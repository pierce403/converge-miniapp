...
The current production alert checkpoint shipped through Cloudflare Workers Builds in commit `6c55a07` and immutable Worker version `d1b3ee8f-c6f3-4610-a1b9-f6b2b556d3dd`. Production returns `{"available":true}`, the manifest contains `https://miniapp.converge.cv/api/farcaster/webhook`, and the live asset contains the fixed `notification_token_pending` diagnostic. The next diagnostics candidate passes the 654-test full gate and production build: it signs the exact returned `signatureText`, requires exact provider and first-party success envelopes, and emits only a structured fixed stage, numeric upstream status, and allowlisted provider code for subscription/revocation failures. Arbitrary response bodies and caught SDK details are neither logged nor rendered. Complete one genuine callback, one closed-app delivery, signed-token rotation, disable/removal, installation replacement, callback replay, invalid-token deletion, upstream throttling, and app-side route revocation acceptance. Review D1 only for ciphertext, opaque handles, and delivery IDs, and inspect sampled logs for token, URL, FID, signature, topic, HMAC, inbox, installation, sender, or message leakage. Reverting production to `"false"` withdraws public availability but does not erase existing encrypted tokens or replace route-level revocation.
