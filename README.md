...
Incoming alerts use Farcaster's native Mini App notification permission plus vapid.party's queue-backed observer of XMTP's global legacy `production` stream. The browser proves its current installation with XMTP public-context Ed25519ph and registers only `Allowed` group topics/HMAC filtering material; message plaintext and decryption keys never enter either backend. vapid.party ignores unmatched, self-authored, and `shouldPush: false` envelopes, then signs an opaque message-available callback containing only a delivery ID and Mini-generated handle. The Mini Worker verifies it, maps that handle to encrypted Farcaster tokens, and sends fixed generic copy to the exact canonical root. `GET /api/notifications/status` stays unavailable and the manifest omits its webhook until every production dependency is configured and the separate exact-string rollout flag is enabled.
