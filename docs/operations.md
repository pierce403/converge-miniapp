...
- Alert promotion is live as of 2026-07-27. The current-Hub, notification-encryption, and three vapid.party secret names are configured; the exact DNS binding resolves publicly; production `FARCASTER_NOTIFICATIONS_ENABLED` is `"true"` while preview remains `"false"`; `GET /api/notifications/status` returns `{"available":true}`; and the manifest publishes the exact canonical webhook. Farcaster's public debugger validates the manifest and ownership proof. Signed lifecycle, installation enrollment, closed-app delivery, and cleanup still require real-host acceptance before the alert milestone is complete.
...
The reviewed config-only promotion shipped through Cloudflare Workers Builds in commit `5f65625` and immutable Worker version `1e270715-1632-4462-9120-8895dd590314`. Production returns `{"available":true}`, the manifest contains `https://miniapp.converge.cv/api/farcaster/webhook`, and Farcaster's public debugger sees that exact value with a valid association. Complete signed add, rotation, disable/removal, token-pending `425`, installation replacement, one closed-app delivery, callback replay, invalid-token deletion, upstream throttling, and app-side route revocation acceptance. Review D1 only for ciphertext, opaque handles, and delivery IDs, and inspect sampled logs for token, URL, FID, signature, topic, HMAC, inbox, installation, sender, or message leakage. Reverting production to `"false"` withdraws public availability but does not erase existing encrypted tokens or replace route-level revocation.
