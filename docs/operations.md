...
- Alert promotion is structurally live as of 2026-07-27. The current-Hub, notification-encryption, and three vapid.party secret names are configured; the exact DNS binding resolves publicly; production `FARCASTER_NOTIFICATIONS_ENABLED` is `"true"` while preview remains `"false"`; `GET /api/notifications/status` returns `{"available":true}`; and the manifest publishes the exact canonical webhook. Farcaster's public debugger validates the manifest and ownership proof. A signed unauthorized-key canary currently returns `503` instead of the expected `400`, proving that the current-Hub provider lookup fails before D1 despite the secret name being present. Replace and validate that credential before treating signed lifecycle, installation enrollment, closed-app delivery, or cleanup as operational.
