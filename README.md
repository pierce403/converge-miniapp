...
Incoming alerts use Farcaster's native Mini App notification permission plus vapid.party's queue-backed observer of XMTP's global legacy `production` stream. The browser proves its current installation with XMTP public-context Ed25519ph and registers its exact installation welcome topic plus `Allowed` and `Unknown` group topics/HMAC filtering material while excluding `Denied`; the welcome topic lets a fresh sender's first conversation wake the closed Mini before that group exists locally. Message plaintext and decryption keys never enter either backend. vapid.party's listener receives global encrypted envelopes, never decrypts or forwards them, ignores unmatched/self-authored HMAC-backed/`shouldPush: false` traffic, then signs the exact callback `{version: 1, type: "xmtp.message_available", deliveryId, inboxHandle}`. The Mini Worker verifies it, maps that opaque handle to encrypted Farcaster tokens, and sends fixed generic copy to the exact canonical root. Production `GET /api/notifications/status` is available and the manifest advertises the canonical webhook; preview remains unavailable and the exact-string rollout flag is the kill switch.
