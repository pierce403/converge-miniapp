...
- Alert promotion is structurally live as of 2026-07-27. The current-Hub, notification-encryption, and three vapid.party secret names are configured; the exact DNS binding resolves publicly; production `FARCASTER_NOTIFICATIONS_ENABLED` is `"true"` while preview remains `"false"`; `GET /api/notifications/status` returns `{"available":true}`; and the manifest publishes the exact canonical webhook. Farcaster's public debugger validates the manifest and ownership proof. After the current-Hub secret update, a signed unauthorized-key canary returns the expected `400`, but two live-tail captures of real disable and enable lifecycle POSTs returned `503` with D1 still empty. Both failed inside the pinned package's generic current-network wrapper. The Worker-native replacement keeps official Farcaster JFS verification and replaces only the Hub lookup/base64/ABI boundary, with fixed provider non-200, response-schema, timeout, request, and app-key metadata categories. It never logs provider messages, request bodies, FIDs, signatures, notification URLs, or tokens. Require a verified real enable event and encrypted D1 row before treating installation enrollment, closed-app delivery, or cleanup as operational.
