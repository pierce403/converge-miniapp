...
- Alert promotion is structurally live as of 2026-07-27. The current-Hub, notification-encryption, and three vapid.party secret names are configured; the exact DNS binding resolves publicly; production `FARCASTER_NOTIFICATIONS_ENABLED` is `"true"` while preview remains `"false"`; `GET /api/notifications/status` returns `{"available":true}`; and the manifest publishes the exact canonical webhook. Farcaster's public debugger validates the manifest and ownership proof. After the current-Hub secret update, a signed unauthorized-key canary returns the expected `400`, but an unsampled live tail captured real disable and enable lifecycle POSTs both returning `503` with D1 still empty. The webhook logs only fixed `configuration`, `verification`, or `lifecycle_apply` failure categories; it never logs provider messages, request bodies, FIDs, signatures, notification URLs, or tokens. Use the category from a fresh live event to isolate the remaining failure before treating installation enrollment, closed-app delivery, or cleanup as operational.
