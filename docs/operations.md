...
- Alert promotion is structurally live as of 2026-07-27. The current-network, notification-encryption, and three vapid.party secret names are configured; the exact DNS binding resolves publicly; production `FARCASTER_NOTIFICATIONS_ENABLED` is `"true"` while preview remains `"false"`; `GET /api/notifications/status` returns `{"available":true}`; and the manifest publishes the exact canonical webhook. Farcaster's public debugger validates the manifest and ownership proof. Live-tail captures of real disable and enable lifecycle POSTs returned `503` with D1 still empty. The Worker-native verifier proved the remaining failure is the outbound request before an HTTP response or metadata decode, and synthetic events reproduce it for both the affected FID and FID 1 on Neynar's legacy Hub and Snapchain API hostnames. The request now handles authenticated redirects manually, forwarding the API key only through a bounded HTTPS redirect within `neynar.com`. Require an unauthorized-key canary to return `400`, followed by a verified real enable event and encrypted D1 row, before treating installation enrollment, closed-app delivery, or cleanup as operational. Fixed diagnostics never log provider messages, request bodies, FIDs, signatures, notification URLs, or tokens.
