...
- Alert promotion is structurally live as of 2026-07-27. The current-Hub, notification-encryption, and three vapid.party secret names are configured; the exact DNS binding resolves publicly; production `FARCASTER_NOTIFICATIONS_ENABLED` is `"true"` while preview remains `"false"`; `GET /api/notifications/status` returns `{"available":true}`; and the manifest publishes the exact canonical webhook. Farcaster's public debugger validates the manifest and ownership proof. After the current-Hub secret update, a signed unauthorized-key canary returns the expected `400`, but live tails captured real disable and enable lifecycle POSTs returning `503` with D1 still empty. Fixed diagnostics isolate the failure to current-network verification and further distinguish provider non-200, response-schema, timeout, and app-key metadata failures; they never log provider messages, request bodies, FIDs, signatures, notification URLs, or tokens. Use the refined category from a fresh live event to repair the verifier before treating installation enrollment, closed-app delivery, or cleanup as operational.
