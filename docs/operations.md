...
| XMTP-to-Farcaster alerts | `PREFERENCES`, `IDENTITY_RATE_LIMITER`, five notification secrets, vapid.party origin, and `FARCASTER_NOTIFICATIONS_ENABLED` | Stores encrypted native tokens plus one random opaque callback handle per verified FID. vapid.party receives global encrypted XMTP envelopes, never decrypts/forwards them, matches only registered routes, and sends signed message-available callbacks; Mini owns native token custody and fixed alert copy. The exact-string rollout variable controls public availability separately from structural configuration. |
...
| ENS identity binding | Browser `localStorage`, namespaced by host-context FID | Journals only the public ENS name, checksummed target/source addresses, expected target inbox ID, and Farcaster signer kind/chain immediately before the first irreversible update. The FID is a non-authoritative namespace hint; every launch independently opens the exact source with Farcaster, requires the saved signer kind (and chain for a smart-contract wallet), and requires fresh network lookups before and after client creation/registration to match the expected inbox ID. Confirmed foreground re-entry, online actions, stream-driven refresh, and push-topic enrollment repeat that check and recreate a stale mounted client against the new assignment. WalletConnect is used and disconnected inside the one-time binding action; pairing URIs, topics, signatures, and keys are never stored. No Worker binding or server row is involved. |
...
- Alert promotion is live through native delivery as of 2026-07-28. The current-network, notification-encryption, and three vapid.party secret names are configured; the exact DNS binding resolves publicly; production `FARCASTER_NOTIFICATIONS_ENABLED` is `"true"` while preview remains `"false"`; `GET /api/notifications/status` returns `{"available":true}`; and the manifest publishes the exact canonical webhook. Farcaster's public debugger validates the manifest and ownership proof. Bounded manual redirects within `neynar.com` repaired current-network verification: Worker `a639f300-5b00-44fd-b675-b9897e4fcfb2` returns `400` for the unauthorized-key canary, and a real enable event created one encrypted native subscription plus one active opaque Mini route. The callback-domain repair then exposed a vapid.party proof bug: XMTP Browser SDK 7 emits public-context Ed25519ph, while vapid.party verified plain Ed25519. vapid.party commit `6fa0c6e` now verifies the exact SDK form with an independent official-libxmtp vector, and the next real Mini attempt created one active HTTPS callback subscription. Production Worker `12865331-37be-455a-b8c0-8938ca501fa3` also runs the rebuilt listener Container with patched gRPC 1.82.1; after reconnect, two control polls remained delivery ready, listener ready, bridge synced, zero pending, and zero failed. After Task 10a refreshed the Mini route, a collaborator-observed fresh-sender test produced the exact fixed native Farcaster copy, proving a genuine end-to-end listener, signed-callback, Mini-delivery, and provider path. Record the Farcaster client/platform, exact alert count, tap target, and resulting conversation before treating gate 5 as fully accepted.
...
The ENS identity release adds protected API routes and D1 state, while its browser-local migration journal never leaves the origin; neither removes the payer-Gateway blocker. Treat the binding as deployed only after the verified `main` build is live and canonical-host discovery still works. Treat it as release-proven only after the exact Reown origin allowlist is confirmed and a production-like dev-network test proves exact-owner QR/same-phone authorization, pre-mutation journal persistence, old-inbox recovery rotation, Farcaster identity reassignment, stateless address-log plus target-state verification, WalletConnect disconnection, Farcaster-only re-entry, and resident-session retargeting on foreground and live-stream activity. Sampled logs must contain no pairing URI or identity values.
...
| `GET /api/notifications/status` | `200 {"available":true}` only when every native and vapid.party dependency is configured **and** `FARCASTER_NOTIFICATIONS_ENABLED` is exactly `"true"`; otherwise `false` | Never returns token, FID, route, app-secret, or structural-configuration state. |
| `POST /api/me/notifications/xmtp-ticket` | Returns vapid.party's separate short-lived ticket, exact `signatureText`, and a server-completed registration after a native token exists | Quick Auth and per-FID rate limit; `425` while the signed Farcaster webhook token is pending; `400/413` invalid bounded topic snapshot; `503` configuration/upstream failure. |
...
| `DELETE /api/me/ens-preference` | `204` after deleting only the verified FID's ENS choice | Does not revoke or delete notification state. |
| `DELETE /api/me` | `204` after upstream route revocation and deletion of every ENS/notification row for the verified FID | Upstream failure returns `503` before local account data is removed. |
...
`POST /api/farcaster/webhook` accepts only bounded JSON on the exact canonical host. The pinned official Mini App server package verifies the Farcaster Signature. A narrow Worker-native verifier then asks the configured Hub for the latest app-key state and decodes the signed-key metadata without Node `Buffer`; Quick Auth and browser context are not substitutes. A verified add/enable event encrypts its URL and token with AES-256-GCM and fresh nonce, binding the ciphertext to the verified user FID, client app FID, canonical domain, and key version. A verified add without details, disable, or removal deletes the exact client row. After the last client row disappears, Mini first marks the FID's opaque XMTP callback handle `revoking`, revokes it through vapid.party's app-secret endpoint, and only then deletes the local handle. An upstream failure returns retryable `503` and retains the tombstone so a concurrent callback is terminal and Farcaster can retry cleanup safely.

After XMTP is ready, the browser rechecks the Farcaster wallet's current XMTP network assignment and sends its bounded snapshot through two Quick Auth routes: exactly one welcome topic matching the current installation with no HMAC keys, plus HMAC-backed group topics for `Allowed` and `Unknown` conversations while excluding `Denied`. Mini rejects a missing, duplicate, malformed, wrong-installation, or HMAC-bearing welcome topic; forces the exact callback URL, generic/minimal-payload preferences, and its own random FID-scoped `inboxHandle`; keeps the vapid.party app secret server-side; and returns vapid.party's exact `signatureText` for `Client.signWithInstallationKey`. The browser signs that text with the current installation, and vapid.party verifies XMTP's RFC 8032 Ed25519ph proof with the fixed `PUBLIC SIGNATURE CONTEXT` before atomically keeping one active installation for `(appId, inboxHandle)`. Plain Ed25519 is incompatible. A welcome-only snapshot is valid for a fresh inbox. The Mini D1 never stores the XMTP inbox ID, installation ID, topics, HMAC keys, proof, ticket, or management receipt.

For delivery, vapid.party's singleton Go process consumes the global legacy XMTP `production` `SubscribeAll` stream. Its current in-memory index contains only registered welcome/topic/HMAC routes. It ignores unmatched or noncanonical topics, `shouldPush: false`, and self-authored HMAC-backed envelopes identified by sender HMAC; it neither decrypts nor forwards the MLS envelope. A match becomes a deduplicated at-least-once Queue job whose only Mini-facing content is a stable delivery ID and opaque handle.

vapid.party callbacks have the exact body `{version: 1, type: "xmtp.message_available", deliveryId, inboxHandle}`. Mini checks the pinned app-ID header separately, then verifies the P-256 signature over the exact timestamp, delivery ID, and raw body. It rejects stale signatures and conflicting delivery-ID/handle reuse, returns `204` for an already delivered exact retry, leases concurrent retries with `425`, decrypts current native tokens, groups at most 100 tokens per exact allowlisted URL, and sends fixed copy to the canonical root. It deletes tokens returned as invalid or permanently invalidated by `target_url_mismatch`, returns retryable status for rate limits and operational outages, and uses the stable delivery ID as Farcaster's notification ID. A missing handle or a handle with no native token returns terminal `410`; vapid.party then removes that logical route without affecting other apps or handles.

The committed non-secret configuration is limited to the reviewed Hub base URL, exact notification delivery URL allowlist, vapid.party origin, and the rollout variable. The closed-app bridge is deliberately production-only because its callback, Quick Auth audience, notification target, and manifest identity are bound to `miniapp.converge.cv`. Its complete secret inventory is one current-Hub credential, one token-encryption key, and three vapid.party app values:
...
Secret-name presence is not a current-Hub acceptance check. After setting
`FARCASTER_HUB_API_KEY`, submit a correctly formed JFS webhook canary signed by
an intentionally unauthorized random app key. Require `400 invalid_request`;
`503 notification_unavailable` means provider authentication, quota,
availability, or response compatibility failed before D1. Only after that
canary passes should a real alerts toggle be used to require one encrypted
subscription row. Never use a real notification token in the canary.
